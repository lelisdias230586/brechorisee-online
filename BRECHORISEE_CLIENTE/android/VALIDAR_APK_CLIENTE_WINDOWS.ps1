...
try {
    ...
    foreach ($entry in $dex) {
        ...
        try {
            ...
            if (
                $text.Contains('Lcom/brechorisee/cliente/MainActivity;') -or
                $text.Contains('com/brechorisee/cliente/MainActivity') -or
                $text.Contains('com.brechorisee.cliente.MainActivity')
            ) {
    ...
    if (!$foundMainActivity) {
        Fail "classes.dex nao contem com.brechorisee.cliente.MainActivity. Esse APK e antigo ou foi compilado do modulo errado."
    }

    # Checagem rapida de certificado v1. O apksigner abaixo e a validacao principal.
    $hasV1Cert = $false
...
Write-Host "OK: APK Cliente assinado, instalavel e com MainActivity correta." -ForegroundColor Green
exit 0
