...
title BRECHORISEE CLIENTE - Gerar APK Release assinado validado
cd /d "%~dp0"

echo.
echo ==============================================
echo  BRECHORISEE CLIENTE - APK RELEASE ASSINADO
echo ==============================================
echo.
echo Este script gera o APK Cliente release assinado para baixar pelo site.
...
if defined BRECHORISEE_CLIENTE_STORE_PASSWORD (
  set "STORE_PASS=!BRECHORISEE_CLIENTE_STORE_PASSWORD!"
  set "KEY_PASS=!BRECHORISEE_CLIENTE_STORE_PASSWORD!"
)

if not exist "%KEYSTORE%" (
  echo.
  echo Nenhuma chave Cliente encontrada.
  echo Vou criar uma nova chave local para o APK Cliente.
  echo.
  echo IMPORTANTE: guarde a pasta play_store_upload em local seguro.
  echo.
  if not defined STORE_PASS call :ASK_NEW_PASSWORD "Cliente"
  ...
  if errorlevel 1 (
    echo ERRO: nao consegui gerar a chave Cliente.
    pause
    exit /b 1
  )
) else (
  echo.
  echo Chave Cliente encontrada:
  echo %KEYSTORE%
  if not defined STORE_PASS call :ASK_EXIST_PASSWORD "Cliente"
)
...
echo.
echo Gerando APK Cliente release...
call "%GRADLE_BIN%" --no-daemon clean assembleRelease
if errorlevel 1 (
  echo.
  echo ERRO: falha ao gerar APK Cliente release.
  pause
  exit /b 1
)

set "APK_SRC=%PROJECT_DIR%\app\build\outputs\apk\release\app-release.apk"
set "APK_DST=%PROJECT_DIR%\BRECHORISEE_CLIENTE_RELEASE.apk"

if not exist "%APK_SRC%" (
  echo.
  echo ERRO: APK Cliente release nao encontrado em:
  echo %APK_SRC%
  pause
  exit /b 1
)
...
echo.
echo Validando APK Cliente...
if not exist "%PROJECT_DIR%\VALIDAR_APK_CLIENTE_WINDOWS.ps1" (
  echo.
  echo ERRO: validador Cliente nao encontrado:
  echo %PROJECT_DIR%\VALIDAR_APK_CLIENTE_WINDOWS.ps1
  pause
  exit /b 1
)
...
if not defined APK_SRC (
  echo.
  echo ERRO: APK_SRC Cliente ficou vazio antes da validacao.
  pause
  exit /b 1
)
if not exist "%APK_SRC%" (
  echo.
  echo ERRO: APK Cliente para validar nao existe:
  echo %APK_SRC%
  pause
  exit /b 1
)
powershell -NoProfile -ExecutionPolicy Bypass -File "%PROJECT_DIR%\VALIDAR_APK_CLIENTE_WINDOWS.ps1" -ApkPath "%APK_SRC%"
if errorlevel 1 (
  echo.
  echo ERRO: APK Cliente nao passou na validacao. Nao sera usado.
  pause
  exit /b 1
)
...
if errorlevel 1 (
  echo.
  echo ERRO: nao consegui copiar o APK Cliente final para:
  echo %APK_DST%
  pause
  exit /b 1
)
...
echo.
echo ==============================================
echo  APK CLIENTE RELEASE GERADO COM SUCESSO
echo ==============================================
echo %APK_DST%
echo.
echo SHA-256 da chave Cliente:
