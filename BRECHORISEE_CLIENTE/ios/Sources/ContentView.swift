...
struct ContentView: View {
    @AppStorage("clienteTutorialSeenV4") private var tutorialSeen = false
    ...
    var body: some View {
            ...
            WebView(url: currentURL)
                ...
                .toolbar {
                    ToolbarItemGroup(placement: .navigationBarTrailing) {
                        ...
                        Button("Live") {
                            currentURL = URL(string: AppConfig.fallbackURL.absoluteString.trimmingCharacters(in: CharacterSet(charactersIn: "/")) + "/cliente/live-opcoes") ?? AppConfig.startURL
                        }
                        Button("Como usar") {
                            currentURL = URL(string: AppConfig.fallbackURL.absoluteString.trimmingCharacters(in: CharacterSet(charactersIn: "/")) + "/cliente/tutorial") ?? AppConfig.startURL
                        }
                    }
                }
        }
        ...
        .onAppear {
            ...
            if !tutorialSeen {
                ...
                currentURL = URL(string: base + "/cliente/tutorial?origem=ios&primeiro=1") ?? AppConfig.startURL
            }
        }
        .onOpenURL { url in
    ...
    private func routeDeepLink(_ url: URL) -> URL {
        ...
        if ["produto", "peca", "product"].contains(host), !id.isEmpty {
            return URL(string: base + "/cliente/peca/" + id) ?? AppConfig.startURL
        }
        if ["tutorial", "como-usar", "ajuda-cliente"].contains(host) {
            return URL(string: base + "/cliente/tutorial") ?? AppConfig.startURL
        }
        if ["live", "aovivo", "ao-vivo"].contains(host) {
            return URL(string: base + "/cliente/live-opcoes") ?? AppConfig.startURL
        }
        if ["sacola", "cart", "carrinho"].contains(host) {
            return URL(string: base + "/loja/carrinho") ?? AppConfig.startURL
        }
...
struct WebView: UIViewRepresentable {
    ...
    final class Coordinator: NSObject, WKNavigationDelegate, WKUIDelegate {
        ...
        private func isBlockedClientPath(_ url: URL) -> Bool {
            let path = url.path.lowercased()
            if path == "/" { return true }
            if path.hasPrefix("/live/peca-atual") || path.hasPrefix("/live/companion") || path.hasPrefix("/live/carrinho") {
                return false
            }
            let blockedPrefixes = [
                "/admin", "/admin-acesso", "/cashier", "/products", "/suppliers", "/sales",
                "/reports", "/stock-history", "/professional", "/profissional", "/gestao",
                "/sincronizacao", "/notificacoes", "/marketing", "/ai", "/ia-clientes",
                "/live", "/labels", "/deliveries", "/clientes-inteligentes", "/loja-admin",
                "/whatsapp-vendas", "/export", "/backups"
            ]
